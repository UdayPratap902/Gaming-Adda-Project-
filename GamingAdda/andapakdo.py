...
# Load the life logo image
# life_image = PhotoImage(file='life_logo.png')
...
score_text = c.create_text(
    30, 30, anchor="nw", font=game_font, fill="darkblue", text="Score: " + str(score))

lives_remaining = 3
lives_text = c.create_text(30, 30, anchor="nw", font=game_font,
                           fill="darkblue", text="\nLives: " + str(lives_remaining))


# life_logo = c.create_image(canvas_width-50,50, image=life_image)
...
def egg_dropped(egg):
    ...
    if lives_remaining == 0:
        pygame.mixer.music.load(
            r"U:\PythonApp\MiniProject\GamingAdda\mixkit-player-losing-or-failing-2042.wav")
        pygame.mixer.music.play(loops=1)
        user_input = messagebox.askquestion(
            "Game Over!", "Final Score: " + str(score))
        if user_input == 'yes':
            os.system(r"E:\projects\New\andapakdo.py")
            root.destroy()
        else:
            root.destroy()
...
def lose_a_life():
    ...
    c.itemconfigure(lives_text, text="\nLives: " + str(lives_remaining))
    pygame.mixer.music.load(
        r"U:\PythonApp\MiniProject\GamingAdda\andagira (1).wav")
    pygame.mixer.music.play(loops=1)
